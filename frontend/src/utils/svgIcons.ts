...
const RAW: Record<SvgIconName, string> = {
...
export function getSvgDataUri(name: SvgIconName, color: string): string {
  const raw = RAW[name].replace(/currentColor/g, color);
  return `data:image/svg+xml;charset=utf-8,${encodeURIComponent(raw)}`;
}

export function getSvgRaw(name: SvgIconName, color: string): string {
  return RAW[name].replace(/currentColor/g, color);
}

export function buildSvgInjectionScript(usedNames: SvgIconName[], colors: Record<string, string>): string {
  const entries = usedNames.map((name) => {
    const uri = getSvgDataUri(name, colors[name] ?? "#ffffff");
    return `"${name}.svg":"${uri}"`;
  });
  return `
<script>
(function(){
  var map = {${entries.join(",")}};
  function patch(img){
    var s = img.getAttribute('src');
    if(s && map[s]) img.src = map[s];
  }
  function init(){
